...
func get_top_card(overlaps):
	for area in overlaps:
		var top = true
		for other in overlaps:
			if other != area:
				if other.is_greater_than(area):
					top = false
					break
		if top:
			return top
...
func get_top_card_overlap():
	...
	prints("overlapped_cards:", overlapped_cards)
	prints("card_group:", card_group)
...
func stack_card(card: Card):
	...
	tween.set_trans(Tween.TRANS_BACK)
	tween.set_ease(Tween.EASE_IN)
	tween.tween_property(card, "position", STACK_OFFSET_VECTOR, .15)
